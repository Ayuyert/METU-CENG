...
##In this one, I again added the JTAB instruction since it is extremely fast for our problem, optimizes the calculating and jumping the desired values in the memory. I implemented it compatible with PIPE logic this time, means, I arranged its stall and bubble locations such that the other instrucions coming after JTAB don't go forward and wait in a stalled position. In this phase, also a bubble is added each clock cycle to decode state. After the new result is calulated, the new instructions will come from there and stall and bubbles will not be added anymore.
#/* $begin pipe-all-hcl */
####################################################################
...
## IJTAB uses Write back stage valM, so we add it here as W_valM
word f_pc = [
